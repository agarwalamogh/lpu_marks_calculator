    ...
    public Looper zzlb() {
        ...
        synchronized (this.zzrJ) {
            if (this.zzYz != 0) {
                ...
                zzpk.m30v("Starting the looper thread.");
                this.zzYy = new HandlerThread("LooperProvider");
                this.zzYy.start();
                this.mHandler = new Handler(this.zzYy.getLooper());
                zzpk.m30v("Looper thread started.");
            } else {
                zzpk.m30v("Resuming the looper thread");
                this.zzrJ.notifyAll();
            }
            this.zzYz++;
                ...
                this.mHandler.post(new Runnable() {
                    public void run() {
                        synchronized (zzpw.this.zzrJ) {
                            zzpk.m30v("Suspending the looper thread");
                            while (zzpw.this.zzYz == 0) {
                                try {
                                    zzpw.this.zzrJ.wait();
                                    zzpk.m30v("Looper thread resumed");
                                } catch (InterruptedException e) {
                                    zzpk.m30v("Looper thread interrupted.");
                                }
                            }
                        }
                    }
